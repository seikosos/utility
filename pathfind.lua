...
Original module Created by: @V3N0M_Z
Rewritten By: seikoso_
Reference: https://v3n0m-z.github.io/RBLX-SimplePath/
reference is basically the same but with 1 extra setting guys!!!!!
License: MIT
...
local function waitforsignal(signal: RBXScriptSignal, timeout: number)
	local bind = Instance.new("BindableEvent")
	local fired = false
	local result

	local sigConn
	sigConn = signal:Connect(function(...)
...
local function move(self)
	...
	local currentWP = self._currentWaypoint
    local completed = false
    local conn = self._humanoid.MoveToFinished:Connect(function(r)
        completed = r
    end)
	repeat
		self._humanoid:MoveTo(self._waypoints[self._currentWaypoint].Position)
		waitforsignal(self._humanoid.MoveToFinished, 0.2)
	until (currentWP ~= self._currentWaypoint) or completed or not self._target or not getmetatable(self)
    conn:Disconnect()
end
...
local function moveToFinished(self, reached)
	...
	if reached and self._currentWaypoint + 1 <= #self._waypoints then --Waypoint reached
		...
		if self._settings.RECOMPUTE_AT_WAYPOINT then
